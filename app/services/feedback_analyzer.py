...
识别关键问题、常见抱怨和任何积极的反馈。
提供一个简洁的报告，总结内容请使用中文，并可以适当在语句前添加相关的emoji来增强表达效果，例如使用 ✅ 表示已解决或积极的方面，使用 ⚠️ 表示需要注意的问题，使用 ❓ 表示疑问或不明确的反馈等。
总结中不要包括用户信息，给正文内容就可以，需要精准描述反馈不要给相关改进意见，如有图片请以[图片](链接)格式给出，切记是总结
反馈条目:
{feedbacks_text}

请直接给总结的内容，不要有任何招呼语或其他内容。
"""
